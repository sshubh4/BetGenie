...
final_df["Date"] = (
    ...
    .str.replace(r"\s*\([^)]*\)", "", regex=True)   # remove "(...)" notes
    .str.replace(r"^\w+,\s*", "", regex=True)       # remove weekday like "Tue, "
    .str.replace(",", "", regex=False)              # remove stray commas
    .str.strip()
    .str.title()                                    # "aug 2" -> "Aug 2"
)

# Make Season numeric, then build a safe string WITHOUT ".0"
final_df["Season"] = pd.to_numeric(final_df["Season"], errors="coerce")
season_str = final_df["Season"].apply(lambda x: str(int(x)) if pd.notna(x) else "")

# Build full_date_str; strip any accidental ".0"
final_df["full_date_str"] = (
    (final_df["Date"].str.strip() + " " + season_str).str.strip()
    .str.replace(r"\.0\b", "", regex=True)
)

# Optional: inspect rows that won't match %b %d %Y
bad_rows = final_df[~final_df["full_date_str"].str.match(r"^[A-Za-z]{3} \d{1,2} \d{4}$")]
if not bad_rows.empty:
    print(bad_rows[["Date", "Season", "full_date_str"]].head(20))

# Parse to YYYY-MM-DD (rows missing Season become NaT)
final_df["Date"] = pd.to_datetime(
    final_df["full_date_str"], format="%b %d %Y", errors="coerce"
).dt.strftime("%Y-%m-%d")
...
print("Hey man its been fun")
print(final_df.columns)
mask_bad = pd.to_datetime(
    final_df["Date"],
    format="%b %d %Y",
    errors="coerce"
).isna()
bad_vals = final_df.loc[mask_bad, "Date"]
print(bad_vals.value_counts().head(20))
