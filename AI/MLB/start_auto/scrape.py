...
class PlaywrightMLBScraper:
                    ...
                    (teamName) => {{
                        ...
                        if (gameInfoText) {{
                            ...
                            if (umpiresSectionMatch) {{
                                ...
                                const parts = umpires.split(/[,;]/);
                                for (const part of parts) {{
                                    const t = part.trim();
                                    if (t.includes('HP')) {{
                                        const name = t.replace(/.*HP[:\\-\\s]*/, '').trim();
                                        data.other_info.umpire_HP = name;
                                    }}
                                    if (t.includes('1B')) {{
                                        const name = t.replace(/.*1B[:\\-\\s]*/, '').trim();
                                        data.other_info.umpire_1B = name;
                                    }}
                                    if (t.includes('2B')) {{
                                        const name = t.replace(/.*2B[:\\-\\s]*/, '').trim();
                                        data.other_info.umpire_2B = name;
                                    }}
                                    if (t.includes('3B')) {{
                                        const name = t.replace(/.*3B[:\\-\\s]*/, '').trim();
                                        data.other_info.umpire_3B = name;
                                    }}
                                    if (t.includes('LF')) {{
                                        const name = t.replace(/.*LF[:\\-\\s]*/, '').trim();
                                        data.other_info.umpire_LF = name;
                                    }}
                                    if (t.includes('RF')) {{
                                        const name = t.replace(/.*RF[:\\-\\s]*/, '').trim();
                                        data.other_info.umpire_RF = name;
                                    }}
                                }}
                            }}
                            
                            # Extract time, attendance, weather
                            const timePatterns = [
                                /Time of Game:\\s*([^\\n.]+)/i,
                                /Game Time:\\s*([^\\n.]+)/i,
                                /Duration:\\s*([^\\n.]+)/i
    ...
    def get_team_name(self, team_code):
        ...
        team_map = {
            ...
            "NYY": "NewYorkYankees", "PHI": "PhiladelphiaPhillies",
    ...
    async def scrape_maintable(self):
        ...
        # If simple logs already exist, go straight to boxscores
        if os.path.exists("simple_team_logs.csv"):
            print("📋 Found existing simple_team_logs.csv - loading data...")
            df = pd.read_csv("simple_team_logs.csv", low_memory=False)
        ...
        mlb_team_map = {
            ...
            "LAA": "LosAngelesAngels", "LAD": "LosAngelesDodgers", "MIA": "MiamiMarlins",
            "MIL": "MilwaukeeBrewers", "MIN": "MinnesotaTwins", "NYM": "NewYorkMets",
            "NYY": "NewYorkYankees", "ATH": "OaklandAthletics", "PHI": "PhiladelphiaPhillies",
        ...
        for team_abbr in mlb_team_map:
            ...
            # ---------- Robust last saved date detection (per team, per season) ----------
            last_date = None
            csv_path = f"./teamgamelogs/{team_abbr}_boxscores.csv"
            if os.path.exists(csv_path):
                try:
                    df_team = pd.read_csv(csv_path, low_memory=False)

                    # Coerce Season to numeric to survive "2025" (string) in CSV
                    df_team['Season'] = pd.to_numeric(df_team.get('Season'), errors='coerce')

                    for season in seasons:
                        # We'll compute per-season last_date inside the season loop
                        pass
                except Exception as e:
                    print(f"↪ {team_abbr}: failed reading {os.path.basename(csv_path)} ({e}); treating as empty.")

            for season in seasons:
                # Refresh last_date per season
                last_date = None
                if os.path.exists(csv_path):
                    try:
                        df_team = pd.read_csv(csv_path, low_memory=False)
                        df_team['Season'] = pd.to_numeric(df_team.get('Season'), errors='coerce')
                        df_year = df_team[df_team['Season'] == season].copy()

                        if not df_year.empty and 'Date' in df_year.columns:
                            # Normalize dates (remove "(1)/(2)" suffixes and commas)
                            s = (
                            ...
                            parsed = pd.to_datetime(s + f" {season}", errors='coerce')
                            if parsed.notna().any():
                                last_date = parsed.max().to_pydatetime()
                                print(f"↪ {team_abbr}: last saved date for {season} = {last_date.date()}")
                            else:
                                print(f"↪ {team_abbr}: could not parse dates; treating as empty for {season}.")
                        else:
                            print(f"↪ {team_abbr}: no rows for Season={season}; treating as empty.")
                    except Exception as e:
                        print(f"↪ {team_abbr}: failed reading {os.path.basename(csv_path)} ({e}); treating as empty.")
                ...
                for game in games:
                    ...
                    base_date_str = re.sub(r"\s*\(\d+\)$", "", raw_date_str).strip()  # strip (1)/(2)

                    # Robust parsing: remove commas and append season
                    g_date = pd.to_datetime(
                        re.sub(",", "", base_date_str) + f" {season}",
                        errors='coerce'
                    )
                    if pd.isna(g_date):
                        continue
                    g_date = g_date.to_pydatetime()

                    # Compare only against normalized last_date (if exists)
                    if last_date and g_date <= last_date:
                        continue

                    # Preserve original date (with DH suffix) in the saved row
                    full_row = game + [season, 0]
                    full_row[1] = raw_date_str  # keep "(1)" or "(2)" in CSV
    ...
    async def scrape_boxscores(self, all_entries, final_headers):
        ...
        for team_code, entries in team_entries.items():
            ...
            # ---------- Safe append + dedupe + sort ----------
            df_new = df.copy()
            if os.path.exists(output_path):
                df_old = pd.read_csv(output_path, low_memory=False)
                df_new = pd.concat([df_old, df_new], ignore_index=True)

            # Optional: sort and dedupe by ['Season','Date','Boxscore URL'] if present
            if {'Season','Date','Boxscore URL'}.issubset(df_new.columns):
                ...
                # Build a parseable date with Season
                _season_str = df_new['Season'].astype(str).str.strip()
                df_new['_parsed_date'] = pd.to_datetime(_d + " " + _season_str, errors='coerce')
                df_new = df_new.sort_values(['Season','_parsed_date']).drop(columns=['_parsed_date'])
                df_new = df_new.drop_duplicates(subset=['Season','Date','Boxscore URL'], keep='last')
        ...
        if all_enhanced:
            ...
            if os.path.exists(first_file):
                sample_df = pd.read_csv(first_file, low_memory=False)
